        ...
        <body id="body-pd">
            <% String userName=null; Cookie[] cookies=request.getCookies(); if(cookies !=null){ for(Cookie cookie :
                ...
                <div class="height-100 bg-light">
                    <div class="container">
                        <h3 class="text-center">List of Appointment</h3>
                        <hr>
                        <div class="container text-left">
                            <a href="<%=request.getContextPath()%>/AppointmentServlet?action=showAddForm"
                                class="btn btn-success">Add New Appointment</a>
                        </div>
                        <br>
                        <table class="table table-bordered Responsive">
                            ...
                            <tbody>
                                ...
                                <c:forEach var="appointmentsList" items="${appointmentsList}">
                                    <tr>
                                        <td>
                                            <c:out value="${appointmentsList.getAppointment_ID()}" />
                                        </td>
                                        <td>
                                            <c:out value="${appointmentsList.getConsultant_name()}" />
                                        </td>
                                        <td>
                                            <c:out value="${appointmentsList.getJobSeeker_name()}" />
                                        </td>
                                        <td>
                                            <c:out value="${appointmentsList.getAppointment_Date()}" />
                                        </td>
                                        <td>
                                            <c:out value="${appointmentsList.getStatus()}" />
                                        </td>
                                        <td>
                                            <c:out value="${appointmentsList.getType()}" />
                                        </td>

                                        <td>
                                            <c:out value="${appointmentsList.getCountry()}" />
                                        </td>

                                        <td>
                                            <c:out value="${appointmentsList.getJob()}" />
                                        </td>

                                        <td><a class="btn btn-primary"
                                                href="AppointmentServlet?action=showUpdateForm&id=<c:out value='${appointmentsList.getAppointment_ID()}' />">Edit</a>
                                            &nbsp;&nbsp;&nbsp;&nbsp; <a class="btn btn-danger"
                                                href="AppointmentServlet?id=<c:out value='${appointmentsList.getAppointment_ID()}' />">Delete</a>
