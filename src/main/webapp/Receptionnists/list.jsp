        ...
        <body id="body-pd">
            <% String userName=null; Cookie[] cookies=request.getCookies(); if(cookies !=null){ for(Cookie cookie :
                ...
                <div class="height-100 bg-light">
                    <div class="container">
                        <h3 class="text-center">List of Receptionist</h3>
                        <hr>
                        <div class="container text-left">
                            <a href="<%=request.getContextPath()%>/ReceptionistServlet?action=createFrm"
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <c:forEach var="receptionist" items="${receptionistsList}">
                                    <tr>
                                        <td>
                                            <c:out value="${receptionist.getReceptionist_ID()}" />
                                        </td>
                                        <td>
                                            <c:out value="${receptionist.getFirst_Name()}" />
                                        </td>
                                        <td>
                                            <c:out value="${receptionist.getLast_Name()}" />
                                        </td>
                                        <td>
                                            <c:out value="${receptionist.getEmail()}" />
                                        </td>
                                        <td>
                                            <c:out value="${receptionist.getPhone_Number()}" />
                                        </td>
                                        <td>
                                            <a href="<%=request.getContextPath()%>/ReceptionistServlet?action=editFrm&id=${receptionist.getReceptionist_ID()}"
                                                class="btn btn-primary">Edit</a>
                                            <a href="<%=request.getContextPath()%>/ReceptionistServlet?action=delete&id=${receptionist.getReceptionist_ID()}"
