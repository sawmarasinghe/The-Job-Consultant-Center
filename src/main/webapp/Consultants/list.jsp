        ...
        <body id="body-pd">
            <% String userName=null; Cookie[] cookies=request.getCookies(); if(cookies !=null){ for(Cookie cookie :
                ...
                <div class="height-100 bg-light">
                    <div class="container">
                        <h3 class="text-center">List of Consultant</h3>
                        <hr>
                        <div class="container text-left">
                            <a href="<%=request.getContextPath()%>/ConsultantsServlet?action=createFrm"
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                <c:forEach var="consultant" items="${consultantsList}">
                                    <tr>
                                        <td>
                                            <c:out value="${consultant.getConsultant_ID()}" />
                                        </td>
                                        <td>
                                            <c:out value="${consultant.getFirst_Name()}" />
                                        </td>
                                        <td>
                                            <c:out value="${consultant.getLast_Name()}" />
                                        </td>
                                        <td>
                                            <c:out value="${consultant.getSpecialization()}" />
                                        </td>
                                        <td>
                                            <c:out value="${consultant.getEmail()}" />
                                        </td>
                                        <td>
                                            <c:out value="${consultant.getPhone_Number()}" />
                                        </td>
                                        <td>
                                            <c:out value="${consultant.getCountries()}" />
                                        </td>
                                        <td>
                                            <c:out value="${consultant.getJobs()}" />
                                        </td>
                                        <td><a class="btn btn-primary"
                                                href="ConsultantsServlet?action=editFrm&id=<c:out value='${consultant.getConsultant_ID()}' />">Edit</a>
                                            &nbsp;&nbsp;&nbsp;&nbsp; <a class="btn btn-danger"
                                                href="ConsultantsServlet?id=<c:out value='${consultant.getConsultant_ID()}' />">Delete</a>
                                            &nbsp;&nbsp;&nbsp;&nbsp; 
                                        </td>
                                    </tr>
                                </c:forEach>
